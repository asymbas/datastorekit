...
public final class DatabaseQueue: Sendable {
    ...
    nonisolated package final let attachment: (any DatabaseAttachment)?
    
    nonisolated package final let makeTransaction:
    @Sendable (any EditingStateProviding, SQLite) -> (any DatabaseTransaction)?
    
    nonisolated internal final let onTransactionFailure:
    @Sendable (borrowing DatabaseConnection) -> Void
    ...
    nonisolated package init(
        writers: Int,
        readers: Int,
        attachment: (any DatabaseAttachment)?,
        makeTransaction:
        @escaping @Sendable (any EditingStateProviding, SQLite) -> (any DatabaseTransaction)?,
        onTransactionFailure:
        @escaping @Sendable (borrowing DatabaseConnection) -> Void,
        makeWriterConnection: (Int) throws -> SQLite,
        makeReaderConnection: (Int) throws -> SQLite
    ) throws {
    ...
    nonisolated public final func close() throws {
        try closeLock.withLock { _ in
            ...
            logger.debug("DatabaseQueue: closing")
...
extension DatabaseQueue {
    ...
    nonisolated public final func connection(
        ...
        let context: (any DatabaseContext)? = {
            switch editingState {
            case let value?: self.attachment?.makeObjectContext(editingState: value)
            case nil: Optional<any DatabaseContext>.none
            }
        }()
        let handle: SQLite
        switch role {
    ...
    nonisolated public final func release(_ connection: consuming DatabaseConnection) {
...
extension DatabaseQueue {
    nonisolated public final func _withConnection<Result>(
        _ role: DataStoreRole? = nil,
        for editingState: (any EditingStateProviding)? = nil,
        _ operation: (borrowing DatabaseConnection) throws -> Result
    ) throws -> Result {
    ...
    nonisolated public final func withConnection<Result>(
        _ role: DataStoreRole? = nil,
        for editingState: (any EditingStateProviding)? = nil,
        _ operation: (inout sending DatabaseConnection) throws -> sending Result
    ) throws -> Result {
...
extension DatabaseQueue {
    package final class ConnectionPool: Sendable {
        ...
        nonisolated private let connections: Mutex<[SQLite]>
        ...
        nonisolated fileprivate func addInitial(_ handle: SQLite) {
            precondition(handle.role == role, "Handle role mismatch for pool.")
            connections.withLock { $0.append(handle) }
            semaphore.signal()
        }
        
        nonisolated fileprivate func acquire(timeout: DispatchTimeInterval) throws -> SQLite {
        ...
        nonisolated fileprivate func release(_ handle: SQLite) {
...
extension DatabaseQueue {
    nonisolated public convenience init(
        ...
        writers: Int,
        readers: Int,
        attachment: (any DatabaseAttachment)? = nil,
        makeTransactionAttachment:
        @escaping @Sendable (any EditingStateProviding, SQLite) -> (any DatabaseTransaction)?,
        onTransactionFailure:
        (@Sendable (borrowing DatabaseConnection) -> Void)? = nil,
