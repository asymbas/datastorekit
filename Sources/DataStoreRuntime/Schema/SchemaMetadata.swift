...
nonisolated internal func createNestedPropertyMetadata<Model>(
    schema: Schema,
    type: Model.Type,
    property: any SchemaProperty
) throws where Model: PersistentModel & SendableMetatype {
    let description = "\(Model.self).\(property.name) as \(property.valueType).self"
    switch property {
    case let compositeAttribute as Schema.CompositeAttribute:
        for _ in compositeAttribute.properties {
    ...
    case let relationship as Schema.Relationship where relationship.isToOneRelationship:
        ...
        guard let relationshipKeyPath = relationship.keypath else {
            return
        }
        try processNestedRelationshipAccess(type: destinationType)
        func processNestedRelationshipAccess<Relationship>(type: Relationship.Type)
        throws where Relationship: PersistentModel & SendableMetatype {
            ...
            for var subAttribute in innerRelationshipProperties  {
                ...
                func buildNestedRelationshipKeyPath<Value>(type: Value.Type) throws {
                    let resolvedKeyPath: AnyKeyPath
                    switch subKeyPath {
                    case let subKeyPath as KeyPath<Relationship, Value>: resolvedKeyPath = subKeyPath
                    case let subKeyPath as KeyPath<Relationship, Value?>: resolvedKeyPath = subKeyPath
                    default: return
                    }
                    guard let constructedKeyPath = relationshipKeyPath.appending(path: resolvedKeyPath),
                          let keyPath: AnyKeyPath & Sendable = sendable(cast: constructedKeyPath) else {
                        return
                    }
                    subAttribute.keyPath = keyPath
                }
            }
        }
    default:
