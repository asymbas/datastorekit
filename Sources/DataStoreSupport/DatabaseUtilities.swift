...
nonisolated public func measure<T>(
    _ function: StaticString = #function,
    _ label: String = "",
    _ block: () throws -> T
) rethrows -> T {
...
nonisolated(nonsending) public func measureAsync<T>(
    _ function: StaticString = #function,
    _ label: String = "",
    _ block: () async throws -> T
) async rethrows -> T {
...
nonisolated public func delay(by timeInterval: TimeInterval = 0.1) {
    #if RELEASE
    #error("This should not be called in release builds.")
    #endif
    Thread.sleep(forTimeInterval: timeInterval)
...
nonisolated public var threadDescription: String {
    let isMainThread = "Main Thread? \(Thread.isMainThread)"
    let isMultiThreaded = "Multiple Threads? \(Thread.isMultiThreaded())"
    let currentThread = "Current Thread: \(Thread.current)"
    let operationQueue = "Operation Queue: \(String(describing: OperationQueue.current))"
...
nonisolated public func printMirrorDetails(
    of value: Any,
    indentLevel: Int = 0,
    label: String? = nil
) {
...
nonisolated public func liftKeyPath<Super, Sub, Value>(
    _ keyPath: KeyPath<Super, Value>,
    to _: Sub.Type
) -> KeyPath<Sub, Value>? {
    guard Sub.self is Super.Type else { return nil }
...
nonisolated public func subclasses<T>(of root: T.Type) -> [T.Type] {
    var count: UInt32 = 0
    guard let classes = objc_copyClassList(&count) else { return [] }
    defer { free(UnsafeMutableRawPointer(classes)) }
    var result: [T.Type] = []
...
nonisolated public func isEqual<T: Equatable>(lhs: Any, rhs: Any, as type: T.Type) -> Bool {
    guard let lhs = lhs as? T, let rhs = rhs as? T else { return false }
    return lhs == rhs
}

nonisolated public func isNil(_ value: Any) -> Bool {
    func unwrap<T>() -> T { return value as! T }
    let unwrappedValue: Any? = unwrap()
    return unwrappedValue == nil
}

nonisolated public func sendable<T: Sendable>(cast value: Any) -> T? {
    value as? T
}

nonisolated public func quote(_ identifier: String) -> String {
    "\"\(identifier.replacingOccurrences(of: "\"", with: "\"\""))\""
}

nonisolated public func combineFlags<T>(_ flags: some Sequence<T>) -> T.RawValue
where T: RawRepresentable, T.RawValue: FixedWidthInteger {
    flags.reduce(0) { $0 | $1.rawValue }
}
